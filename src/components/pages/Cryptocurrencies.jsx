...
const Cryptocurrencies = ({ simplified }) => {
  ...
  const filteredData = cryptoList?.data?.coins.filter((coin) =>
    coin.name.trim().toLowerCase().includes(searchTerm.trim().toLowerCase())
  );
  ...
  return (
    <>
      <>
        ...
        {currentItems?.length <= 0 && searchTerm.length > 0 && !isFetching ? (
          <div className="w-full text-center text-lg inline-block ">
            <p className="text-slate-600 inline-flex">
              no result found for
              <p className="text-slate-800 font-semibold ml-1">
                "{searchTerm}"
              </p>
            </p>
          </div>
                ...
                >
                  <Link to={`/crypto/${item?.uuid}`}>
                    <Card
                      ...
                      extra={
                        <div className="flex gap-2 items-center text-xs sm:text-sm text-zinc-600 ml-1">
                          <p>{item?.symbol}</p>
