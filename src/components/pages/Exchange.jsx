...
const Exchange = () => {
    ...
    const { data: exchengeData, isFetching, isLoading } = useGetExchangesQuery({ coinId: selectExchenge, limit: selectLimit })
    ...
    if (cryptoList === undefined || isLoading) return <Loading text="Loading..." />
    ...
    return (
        <div>
            ...
            <div className=' w-full   md:px-8 px-1 mb-5 gap-2 flex items-center'>
                <div className='ml-2  lg:flex-[0.2] md:flex-[0.3] flex-[0.5]  flex-row'>
                    <h4 className=' text-slate-600 md:text-base text-sm'>Exchenge for:</h4>
                    ...
                    >
                        ...
                        {cryptoList?.data?.coins.map(name => <option value={name.uuid} label={name.name} >{name.name}</option>)}
                ...
                <div className='ml-2 lg:flex-[0.2] md:flex-[0.3]  flex-[0.5]  flex-row'>
                    ...
                    >
                        ...
                        <option disabled={disOption} value={25} >25</option>
                        <option disabled={disOption} value={50} >50</option>
                        <option disabled={disOption} value={75} >75</option>
                        <option disabled={disOption} value={100} >100</option>
                        {/* {num.map(item=>
      <option  disabled={exchenges?.exchanges?.length < selectLimit ?:false} value={item} >{item}</option>
            ...
            <div className='flex flex-col w-full my-4 sm:gap-4 gap-2  '>
                ...
                <div className='flex sm:px-4 px-0 w-full   justify-center   '>
                    <div className='w-full lg:w-10/12 bg-slate-200 flex justify-between items-center rounded lg:p-4  md:p-3 sm:p-3 p-3 gap-4 text-slate-600 font-bold  lg:text-sm md:text-xs text-xs  '>
                        <span className='sm:flex-[0.2] xl:ml-2 2xl:text-left text-center'>Exchenges</span>

                        <span className='sm:flex-[0.2]  text-center '>24h Trade Volume</span>
            ...
            {isFetching ? <ChartLoad /> :


                <div className='flex flex-col w-full my-4 gap-4 '>
                    {exchenges?.exchanges?.map(item =>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='w-full' transition={{ delay: 0.25, duration: 0.40, ease: "easeOut" }}>

                            <div className='flex sm:px-4 px-0 w-full   justify-center   ' key={item.uuid}>
                                <div className=' exchenge-reponsive w-full lg:w-10/12 flex justify-between items-center sm:p-4 p-2 sm:gap-4 gap-2 !font-semibold border-b '>
                                    ...
                                    <div className=' flex-[0.2] text-center flex items-center gap-2 sm:gap-3 '>
                                        ...
                                        <div className='sm:w-10 sm:h-10 w-6 h-6 overflow-hidden rounded  relative flex  items-center transition-all '>
                                            <img className='w-full h-full object-contain' src={item?.iconUrl} alt={item?.iconUrl} />
